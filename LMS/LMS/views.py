...
def HOME(request):
    header = Categories.objects.all().order_by('id')[0:5]
...
def SEARCH_MY_COURSE(request):
    category = Categories.get_all_category(Categories)
    query = request.GET['query']
    course1 = Course.objects.filter(title__icontains=query)
    ...
    return render(request, 'search/search_my_course.html', context)